...
export default function WatchPage({ params }: { params: { videoId: string } }) {
  ...
  const shareVideo = async () => {
    ...
    const title = video?.title ?? "Influence TV";
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-6">
      ...
      <div className="mt-4">
        ...
        <p className="mt-1 text-sm text-itv-muted">{video?.creator_name ?? "Influence TV"} · {formatCount(video?.view_count)} views</p>
        <div className="mt-3 flex gap-2">
          <Button variant="ghost" className="text-xs" onClick={shareVideo}>Share</Button>
        </div>
        {video?.description && <p className="mt-4 whitespace-pre-line text-sm text-itv-text">{video.description}</p>}
      </div>
